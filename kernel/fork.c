...
static struct mm_struct *mm_init(struct mm_struct *mm, struct task_struct *p)
{
    ...
    bitmap_zero(mm->smv_bitmapInUse, SMV_ARRAY_SIZE);
...
static int dup_mmap_smv(struct mm_struct *mm, int smv_id)
{
	struct vm_area_struct *mpnt, *tmp, *prev, **pprev;
	struct rb_node **rb_link, *rb_parent;
	int retval;
	...
	rb_link = &mm->mm_rb_smv[smv_id].rb_node;
	rb_parent = NULL;
	pprev = &mm->mmap_smv[smv_id];

	prev = NULL;
	for (mpnt = mm->mmap_smv[MAIN_THREAD]; mpnt; mpnt = mpnt->vm_next) {
		struct file *file;
		...
		if (mpnt->vm_flags & VM_ACCOUNT) {
			...
			if (security_vm_enough_memory_mm(mm, len)) /* sic */
				goto fail_nomem;
			charge = len;
		}
		tmp = kmem_cache_alloc(vm_area_cachep, GFP_KERNEL);
		...
		*pprev = tmp;
		...
		rb_parent = &tmp->vm_rb;
                ...
                // let the fault handler copy the page table entries
		//retval = copy_page_range(mm, mm, mpnt);
...
static int copy_mm(unsigned long clone_flags, struct task_struct *tsk)
{
	...
	if (clone_flags & CLONE_VM) {
		...
		mm = oldmm;
                // copy the vmas from the main thread to the smv thread
                if (mm->using_smv && tsk->smv_id > MAIN_THREAD) {
                    retval = dup_mmap_smv(mm, tsk->smv_id);
                    if (retval)
                        goto fail_nomem;
                }
		goto good_mm;
	      ...
	      unsigned long tls)
{
	...
	if (!IS_ERR(p)) {
        ...
        /* Reset smv_id for smv_thread_create and assign smv_id
           to the child task p.
           User space guarantees the atomic operation of
           forking smv threads */
        if (current->mm) {
